...
class MatchRepository(var api: RemoteApi, db: MatchDataBase) {
    ...
    suspend fun readFromDataBase(id: String,puuid: String): MatchModel {
        ...
        matchModel.info.participants?.firstOrNull { it.puuid == puuid }?.assists=matchData?.assists0
        matchModel.info.participants?.firstOrNull { it.puuid == puuid }?.kills=matchData?.kills0
        matchModel.info.participants?.firstOrNull { it.puuid == puuid }?.win=matchData?.win0?:true
        return matchModel
    }
}